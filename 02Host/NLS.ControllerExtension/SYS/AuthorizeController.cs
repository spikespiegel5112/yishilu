...
namespace NLS.Host.ControllerExtension.SYS
{
    ...
    public sealed class AuthorizeController : NLSBaseController
    {
        ...
        /// <returns></returns>
        [AllowAnonymous]
        [HttpGet("h5.wxuser.pageaccess")]
        public JsonResult PageAccess(int u_id, int scan_type)
        {
            var result = sysUserService.PageAccess(u_id, scan_type);
            if (result.State == ResultStatusCode.Success)
            {
                return Result(result);
            }
        ...
        /// <returns></returns>
        [AllowAnonymous]
        [HttpGet("h5.get.wxuser.info")]
        public JsonResult GetUserInfo(int u_id)
        {
            var result = sysUserService.GetUserInfo(u_id);
            return Result(result);
        }
