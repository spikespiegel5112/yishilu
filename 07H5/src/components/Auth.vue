...
export default {
  ...
  created() {
    ...
    this.environment = this.$isEmpty(this.$webStorage.getItem("environment"))
      ? this.$checkEnvironment()
      : this.$webStorage.getItem("environment");
    ...
    if (this.environment === "wechat") {
      ...
      this.getUserInfoByOpenId("oPxr9wlKa8Gbr-dxJwWx4GSqG_1g")
        .then((response) => {
          if (response.data) {
  ...
  methods: {
    ...
    getUserInfoByOpenId(openId) {
      return new Promise((resolve, reject) => {
        this.$http
          .get(this.$baseUrl + "wx.login.user.byopenid", {
