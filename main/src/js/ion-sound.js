 ...
 * © Denis Ineshin, 2016
 *
 * Project page:    http://ionden.com/a/plugins/this.sound/en.html
 * GitHub page:     https://github.com/IonDen/this.sound
 *
...
window.ion=function(){
  ...
  if (typeof Audio !== "function" && typeof Audio !== "object") {
    ...
    this.sound = func;
    this.sound.play = func;
    this.sound.stop = func;
    this.sound.pause = func;
    this.sound.preload = func;
    this.sound.destroy = func;
    func();
    return;
  }
  ...
  this.sound.VERSION = "3.0.7";

  this.sound._method = function (method, name, options) {
    if (name) {
  ...
  this.sound.preload = function (name, options) {
    options = options || {};
    extend({preload: true}, options);

    that.sound._method("init", name, options);
  };

  this.sound.destroy = function (name) {
    that.sound._method("destroy", name);
  ...
  this.sound.play = function (name, options) {
    that.sound._method("play", name, options);
  };

  this.sound.stop = function (name, options) {
    that.sound._method("stop", name, options);
  };

  this.sound.pause = function (name, options) {
    that.sound._method("pause", name, options);
  };

  this.sound.volume = function (name, options) {
    that.sound._method("volume", name, options);
  };
