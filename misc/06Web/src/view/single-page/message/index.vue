<template>
  <Card shadow>
     <Row class="margin_bottom_15">
        <Col span="3"  >
        <Select placeholder="消息类型" :transfer="true" v-model="push_type" @on-change="changeartype()">
      ...
      <Select  :transfer="true" v-model="push_type_child" @on-change="getsysmsgpagelist(1)">
            <Option v-for="item in pointtypelist" :key="item.key" :value="item.key">{{item.value}}</Option>
        </Select>
      </Col>
         <Col span="3" style="padding-left: 25px;">
         <Button
    ...
    <div>
      <div class="message-page-con message-category-con">
        <Menu width="auto" active-name="unread" @on-select="handleSelect">
          ...
          <MenuItem name="readed">
            <span class="category-title">已读消息</span><Badge style="margin-left: 10px" class-name="gray-dadge"></Badge>
          </MenuItem>
          <!-- <MenuItem name="trash">
      ...
      <div class="message-page-con message-list-con" style="height:100%">
        ...
        > 
          ...
          <Badge v-if="recive_type==1" dot :count="item.f_judicial_pc_read==0?1:0">
            <div > 
              <p class="msg-title" style="width: 200px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">{{ item.p_name }}:{{ item.push_content }}</p>
              <Badge  status="default" :text="item.create_date" />
            </div>
            </Badge>
             <Badge v-else-if="recive_type==2" dot :count="item.f_police_pc_read==0?1:0">
            <div>
              <p class="msg-title" style="width:200px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">{{ item.p_name }}:{{ item.push_content }}</p>
              <Badge  status="default" :text="item.create_date" />
            </div>
          ...
          <Page v-if="totalcount>0" :current="pageindex" :total="totalcount" simple  @on-change="getsysmsgpagelist"/>
            <div v-if="totalcount==0" style="text-align: center">暂无消息</div>
        </Menu>
      </div>
       
      <div class="message-page-con message-view-con" style="text-align: center;top:20%">
        <Spin fix v-if="contentLoading" size="large"></Spin>
        <div class="message-view-header">
          <h2 class="message-view-title">{{ showingMsgItem.push_msg }}</h2>
          <time class="message-view-time">{{ showingMsgItem.create_date }}</time>
        </div>
...
export default {
  ...
  methods: {
    ...
    handleSelect (name) {
      ...
      changeartype(){
          if(this.push_type==5){
           this.pointtypelist=[{key:1,value:"超出规定范围"},{key:2,value:"每日未签到违规"},{key:3,value:"周报道未签到违规"},{key:4,value:"集中学习未签到违规"},{key:5,value:"逾期未销假违规"},{key:6,value:"任务未完成"}];   
          }
          else if(this.push_type==6){
              this.pointtypelist=[{key:1,value:"请假待审批"},{key:2,value:"就医报告待审批"},{key:3,value:"违规待处理通知"}];
          }
